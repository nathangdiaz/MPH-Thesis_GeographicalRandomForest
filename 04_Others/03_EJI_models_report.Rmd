...
for (i in 1:10) {
  ...
  rf_mod2 = train(rpl_eji ~ ., data = df, method = "rf", 
                  ntree = 500, # Start with a default number of trees
                  trControl = trainControl(method = "cv", number = 10),
                  tuneGrid = expand.grid(mtry = c(9:15)))
...
for (ntree in ntree_values) {
    rf_maxtrees = train(rpl_eji ~ ., data = df, method = "rf",
...
# Create the line graph
a = ggplot(metrics_df, aes(x = ntree, y = RMSE, color = as.factor(mtry))) +
  geom_line() +
  geom_point(size = 2) +
...
b = ggplot(metrics_df, aes(x = ntree, y = MAE, color = as.factor(mtry))) +
  geom_line() +
  geom_point(size = 2) +
...
c = ggplot(metrics_df, aes(x = ntree, y = Rsquared, color = as.factor(mtry))) +
  geom_line() +
  geom_point(size = 2) +
  labs(y = "R-squared",
       color = "mtry") +
...
temp = SpatialML::grf.bw(rpl_eji ~ e_ozone + e_pm + e_dslpm + e_totcr +
                           ...
                           ep_asthma + ep_cancer + ep_mhlth + ep_diabetes, 
                             ...
                             bw.max = 50,
                             coords = df_coords,
                             trees = rf_mod3$finalModel$ntree, 
                             mtry = rf_mod3$finalModel$mtry, 
                             step = 1)
...
gwrf_mod2 = SpatialML::grf(rpl_eji ~ e_ozone + e_pm + e_dslpm + e_totcr +
                         ...
                         ntree = rf_mod3$finalModel$ntree, 
                         mtry = rf_mod3$finalModel$mtry) 
```
...
mse5 = mean((df$rpl_eji - predictions5)^2)
rmse5 = sqrt(mse5)
mae5 = sum(abs(df$rpl_eji - predictions5))/length(predictions5)
r_squared5 = 1 - sum((df$rpl_eji - predictions5)^2) / sum((df$rpl_eji - mean(df$rpl_eji))^2)
...
# Predicted 1:1 Plot
ggplot(df, aes(x = rf_pred, y = rpl_eji)) +
  geom_point() + 
  theme_bw() + 
...
a = as.data.frame(randomForest::partialPlot(rf_mod3$finalModel, df, ep_diabetes, plot = FALSE))
b = as.data.frame(randomForest::partialPlot(rf_mod4$finalModel, df, e_tri, plot = FALSE))
c = as.data.frame(randomForest::partialPlot(rf_mod4$finalModel, df, e_totcr, plot = FALSE))
d = as.data.frame(randomForest::partialPlot(rf_mod4$finalModel, df, ep_limeng, plot = FALSE))

# Convert the base R plots to ggplot objects
plot_a <- ggplot(a, aes(x, y)) + geom_line() + labs(title = "Partial Dependence of e_tri")
plot_b <- ggplot(b, aes(x, y)) + geom_line() + labs(title = "Partial Dependence of ep_diabetes")
plot_c <- ggplot(c, aes(x, y)) + geom_line() + labs(title = "Partial Dependence of e_totcr")
plot_d <- ggplot(d, aes(x, y)) + geom_line() + labs(title = "Partial Dependence of ep_limeng")
...
# Predicted 1:1 Plot
ggplot(eji_df, aes(x = grf_pred, y = rpl_eji)) +
  geom_point() + 
  theme_bw() + 
  geom_abline(slope=1, intercept=0,linetype="dashed",size=0.5) + 
  geom_smooth(method = "lm", se = FALSE, colour="black",size=0.5) + 
...
a = ggplot(eji_df, aes(fill = rpl_eji)) +
  geom_sf() + 
  theme_void() +
...
b = ggplot(eji_df, aes(fill = grf_pred)) +
  geom_sf() + 
  theme_void() +
  scale_fill_viridis_c() + 
...
for (var_name in variable_names) {
  ...
  p <- ggplot(eji_df, aes_string(fill = var_name)) +
    geom_sf() +
    scale_fill_viridis_c() +
    labs(title =  var_name,
         fill = "Importance") +
...
ggarrange(plot_lst[["e_ozone"]], plot_lst[["e_pm"]])
ggarrange(plot_lst[["e_dslpm"]], plot_lst[["e_totcr"]])
ggarrange(plot_lst[["e_npl"]], plot_lst[["e_tri"]])
ggarrange(plot_lst[["e_tsd"]], plot_lst[["e_rmp"]])
ggarrange(plot_lst[["e_coal"]], plot_lst[["e_lead"]])
ggarrange(plot_lst[["e_park"]], plot_lst[["e_houage"]])
ggarrange(plot_lst[["e_wlkind"]], plot_lst[["e_rail"]])
ggarrange(plot_lst[["e_road"]], plot_lst[["e_airprt"]])
ggarrange(plot_lst[["e_impwtr"]], plot_lst[["ep_minrty"]])
ggarrange(plot_lst[["ep_pov200"]], plot_lst[["ep_nohsdp"]])
ggarrange(plot_lst[["ep_unemp"]], plot_lst[["ep_renter"]])
ggarrange(plot_lst[["ep_houbdn"]], plot_lst[["ep_uninsur"]])
ggarrange(plot_lst[["ep_noint"]], plot_lst[["ep_age65"]])
ggarrange(plot_lst[["ep_age17"]], plot_lst[["ep_disabl"]])
ggarrange(plot_lst[["ep_limeng"]], plot_lst[["ep_mobile"]])
ggarrange(plot_lst[["ep_groupq"]], plot_lst[["ep_bphigh"]])
ggarrange(plot_lst[["ep_asthma"]], plot_lst[["ep_cancer"]])
ggarrange(plot_lst[["ep_mhlth"]], plot_lst[["ep_diabetes"]])
```
