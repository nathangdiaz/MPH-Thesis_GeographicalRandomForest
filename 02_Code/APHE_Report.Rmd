...
### Identification of Missing Tracts

```{r, echo = FALSE}
print("10 Census Tracts that appear in the Census Files but do not appear in the SVI")
# anti_join(x = tracts, y = df_svi, by = c("GEO_ID" = "FIPS")) %>% as.data.frame() %>% select(GEO_ID) 
```
...
```{r, echo = FALSE}
print("4 Census Tracts that appear in the Census Files but do not appear in the SVI")
#anti_join(x = tracts, y = df_eji, by = c("GEO_ID" = "FIPS")) %>% as.data.frame() %>% select(GEO_ID)
```
...
c = ggplot(data = eji_map, aes(fill = RPL_HVM)) + s
